...
#### UC-008: Delete Principal (GDPR)

**Actor:** Compliance Officer or Automated Process

**Goal:** Permanently delete principal data per GDPR right to erasure

**Preconditions:**
- Principal exists and is INACTIVE
- Principal has been INACTIVE for minimum retention period (30 days)
- Caller has permission `iam.principal.gdpr:delete`
- No legal hold on principal data

**Main Flow:**
1. Compliance officer initiates GDPR deletion request
2. iam.principal validates:
   - Principal is INACTIVE
   - Minimum retention period has passed
   - No legal hold flag set
3. iam.principal anonymizes principal data:
   - username → "deleted_user_{uuid}"
   - email → "deleted_{timestamp}@example.com"
   - Clear all PII from PrincipalProfile
   - context_tags → null
4. iam.principal updates Principal:
   - status = DELETED
5. iam.principal deletes from Keycloak
6. iam.principal publishes `iam.principal.principal.deleted` event with pseudonymized ID
7. iam.audit subscribes and logs GDPR deletion with original principal_id (for audit trail)
8. iam.authz subscribes and removes all authorization data

**Postconditions:**
- Personal data anonymized
- Audit trail preserved with pseudonymized identifier
- Principal cannot be recovered
- Business domains notified to anonymize their references

**GDPR Compliance:**
- Personal data removed
- Audit trail preserved (legal requirement)
- Cannot be reversed
- Downstream systems must anonymize their data
...

{
  ...
  "requestor_email": "compliance@example.com"
}
```
...
```json
{
  ...
  "deleted_at": "2025-12-12T15:30:00Z"
}
```

---

#### UC-009: Rotate Service Principal Credentials

**Actor:** Automated Job or Administrator

**Goal:** Rotate API key for service principal (security best practice)

**Preconditions:**
- Principal is SERVICE type
- Principal is ACTIVE
- Rotation date is due OR administrator forces rotation

**Main Flow:**
1. Rotation job (or admin) initiates credential rotation
2. iam.principal validates:
   - Principal is SERVICE type
   - Principal is ACTIVE
3. iam.principal generates new API key
4. iam.principal hashes new API key
5. iam.principal updates ServicePrincipal:
   - api_key_hash = new hash
   - credential_rotation_date = today + 90 days
   - rotated_at = now
6. iam.principal publishes `iam.principal.credentials.rotated` event
7. Service subscribes and updates its stored API key
8. iam.principal returns new API key to administrator (if manual)

**Postconditions:**
- New API key active
- Old API key invalid
- Service must update its configuration
- Next rotation scheduled in 90 days

**Grace Period:**
- Old API key valid for 24 hours to allow rollout
- After 24 hours, old key rejected

**API Request:**
```http
POST /api/iam/principal/v1/principals/{principalId}/rotate-credentials
Content-Type: application/json
...
#### UC-010: Search Principals

**Actor:** Administrator or External System

**Goal:** Find principals matching search criteria

**Preconditions:**
- Caller has permission `iam.principal:search`
- Caller is scoped to specific tenants

**Main Flow:**
1. Caller provides search criteria
2. iam.principal validates permissions
3. iam.principal applies tenant filter based on caller's access
4. iam.principal executes search query
5. iam.principal returns paginated results
...
- status
- primary_tenant_id
- context_tags (JSON query)
- created_at range
- last_login_at range

**API Request:**
```http
GET /api/iam/principal/v1/principals?search=john&principal_type=HUMAN&status=ACTIVE&tenant_id=tenant-123&page=1&size=50
```
  ...
  "items": [
    {
      ...
      "status": "ACTIVE",
      "primary_tenant_id": "tenant-123",
      "last_login_at": "2025-12-12T09:00:00Z"
    }
  ],
  "total": 1,
  "page": 1,
...
#### UC-011: Get Principal by ID

**Actor:** Administrator, External System, or Authorized Service

**Goal:** Retrieve principal details by ID

**Preconditions:**
- Principal exists
- Caller has permission `iam.principal:read`
- Caller has access to the principal's tenant

**Main Flow:**
1. Caller provides principal ID
2. iam.principal validates:
   - Principal exists
   - Caller has permission
   - Caller has access to principal's tenant
3. iam.principal retrieves principal from database
4. iam.principal returns principal details (excluding sensitive data)

**Postconditions:**
- Principal details returned
- No state changes (read-only operation)

**API Request:**
```http
GET /api/iam/principal/v1/principals/{principalId}
```
...
#### UC-012: Update Principal Common Attributes

**Actor:** Administrator

**Goal:** Update non-profile common attributes (context_tags, primary_tenant_id) on any concrete principal type

**Implementation Note:**
Since `Principal` is an abstract base class, this use case does NOT operate on a `Principal` entity directly. The implementation must:
1. Determine the concrete principal type (HUMAN, SERVICE, or SYSTEM) from the principal_id
2. Load the entity from the appropriate repository (HumanPrincipalRepository, ServicePrincipalRepository, or SystemPrincipalRepository)
3. Update the inherited common attributes on the concrete entity
4. Save via the concrete repository

**Preconditions:**
- Principal exists in one of the concrete tables (human_principals, service_principals, or system_principals) and is not DELETED
- Caller has permission `iam.principal:update`

**Main Flow:**
1. Caller provides principal ID and update data
2. iam.principal determines concrete principal type by querying each repository
3. iam.principal validates:
   - Principal exists in concrete table
   - Caller has permission
   - Immutable fields not modified (username, principal_type)
4. iam.principal updates allowed common attributes on the concrete entity:
   - context_tags
   - primary_tenant_id (if new tenant membership exists)
5. iam.principal saves via the appropriate concrete repository
6. iam.principal returns updated principal
...
|-------|-----------|-------|
| context_tags | ✅ Yes | Max 10KB |
| primary_tenant_id | ✅ Yes | Must have ACTIVE membership in new tenant |
| username | ❌ No | Immutable |
| principal_type | ❌ No | Immutable |
| email | ❌ No | Changed via separate flow |

**API Request:**
```http
PATCH /api/iam/principal/v1/principals/{principalId}
Content-Type: application/json
...
**Goal:** Retrieve profile information for a human principal

**Preconditions:**
- Principal exists and is HUMAN type
- Caller is either the principal themselves OR has permission `iam.principal.profile:read`

**Main Flow:**
1. Caller provides principal ID
2. iam.principal validates:
   - Principal exists and is HUMAN
   - Caller has permission (self or admin)
3. iam.principal retrieves profile from database
4. iam.principal returns profile details
...
**API Request:**
```http
GET /api/iam/principal/v1/principals/{principalId}/profile
```
...
```json
{
  ...
  "timezone": "America/New_York",
  "locale": "en-US",
  "avatar_url": "https://dms.example.com/avatars/prin-789.jpg",
...
#### UC-014: Get Credential Status

**Actor:** Administrator or Service Principal Owner

**Goal:** Check credential status for service/system principal (rotation date, expiry)

**Preconditions:**
- Principal exists and is SERVICE or SYSTEM type
- Caller has permission `iam.principal.credentials:read`

**Main Flow:**
1. Caller provides principal ID
2. iam.principal validates:
   - Principal exists
   - Principal is SERVICE or SYSTEM type
   - Caller has permission
3. iam.principal retrieves credential metadata (NOT the credentials themselves)
4. iam.principal returns credential status
...
**API Request:**
```http
GET /api/iam/principal/v1/principals/{principalId}/credentials/status
```
...
```json
{
  ...
  "last_rotated_at": "2025-12-12T10:30:00Z",
...
#### UC-015: List Tenant Memberships

**Actor:** Administrator or Principal (self)

**Goal:** List all tenant memberships for a principal

**Preconditions:**
- Principal exists
- Caller has permission `iam.principal:read` or is the principal
...
2. iam.principal validates:
   - Principal exists
   - Caller has permission
3. iam.principal retrieves all tenant memberships
4. iam.principal returns paginated list of memberships
...
**API Request:**
```http
GET /api/iam/principal/v1/principals/{principalId}/tenants?page=1&size=50
```
...
#### UC-016: Add Tenant Membership

**Actor:** Administrator

**Goal:** Grant a principal access to an additional tenant

**Preconditions:**
- Principal exists and is ACTIVE
- Tenant exists
- No existing ACTIVE membership for this principal-tenant combination
- Caller has permission `iam.principal.tenant:assign`

**Main Flow:**
1. Caller provides principal ID and tenant membership data
2. iam.principal validates:
   - Principal exists and is ACTIVE
   - Tenant exists
   - No duplicate ACTIVE membership
   - Caller has permission
3. iam.principal creates PrincipalTenantMembership record:
   - status = ACTIVE
   - valid_from = provided or today
   - valid_to = provided or null (no expiry)
4. iam.principal returns created membership

**Postconditions:**
- New tenant membership created
- Principal can access new tenant's resources (subject to authorization)

**API Request:**
```http
POST /api/iam/principal/v1/principals/{principalId}/tenants
Content-Type: application/json
...
#### UC-017: Remove Tenant Membership

**Actor:** Administrator

**Goal:** Remove a principal's access to a tenant

**Preconditions:**
- Principal exists
- Membership exists and is ACTIVE
- Cannot remove primary tenant membership (must change primary first)
- Caller has permission `iam.principal.tenant:remove`

**Main Flow:**
1. Caller provides principal ID and tenant ID
2. iam.principal validates:
   - Membership exists
   - Not the primary tenant membership
   - Caller has permission
3. iam.principal updates membership:
   - status = EXPIRED
   - valid_to = now (if not already set)
4. iam.principal returns confirmation

**Postconditions:**
- Membership marked as EXPIRED
- Principal loses access to tenant's resources

**Business Rules:**
- Cannot remove primary tenant membership
- To change primary tenant: first add new tenant, update primary_tenant_id, then remove old

**API Request:**
```http
DELETE /api/iam/principal/v1/principals/{principalId}/tenants/{tenantId}
```
...
#### UC-020: Cross-Tenant Principal Search (Administrative)

**Actor:** Platform Administrator

**Goal:** Search and query principals across all tenants for administrative purposes

**Preconditions:**
- Caller has permission `iam.admin:cross_tenant`
- Caller is a Platform Administrator (not tenant-scoped)

**Main Flow:**
1. Administrator provides search criteria (username, email, status, principal_type, etc.)
2. iam.principal validates:
   - Caller has `iam.admin:cross_tenant` permission
   - Search parameters are valid
3. iam.principal searches across ALL tenant contexts:
   - Queries all principal tables (human_principals, service_principals, system_principals, device_principals)
   - Does NOT apply tenant context filtering
4. iam.principal returns paginated results with tenant information

**Postconditions:**
- Search results returned from all tenants
- No state changes (read-only operation)

**Security Requirements:**
- This endpoint MUST be protected by `iam.admin:cross_tenant` permission
- Audit logging MUST capture all cross-tenant queries
- Results MUST include tenant_id for each principal

**API Request:**
```http
GET /api/iam/principal/v1/admin/principals?status=ACTIVE&principal_type=HUMAN&page=1&size=50
  ...
  "items": [
    {
      ...
      "principal_type": "HUMAN",
      "username": "john.doe",
      "email": "john.doe@tenant-a.com",
      "status": "ACTIVE",
      "primary_tenant_id": "tenant-a",
      "primary_tenant_name": "Tenant A Corp"
    },
    {
      "principal_id": "prin-002",
      "principal_type": "HUMAN",
      "username": "jane.smith",
      "email": "jane.smith@tenant-b.com",
      "status": "ACTIVE",
      "primary_tenant_id": "tenant-b",
      "primary_tenant_name": "Tenant B Inc"
    }
  ],
  "total": 15234,
  "page": 1,
...
**Use Cases:**
- Platform-wide principal reporting
- Security incident investigation across tenants
- Compliance audits
...
#### Principal Creation Flow

```mermaid
sequenceDiagram
    participant EXT as External System<br/>(HR, Portal)
    participant API as iam.principal API
    participant DB as Database
    participant KC as Keycloak
    participant MQ as RabbitMQ
    
    EXT->>API: POST /principals<br/>{type: HUMAN, email, username}
    API->>API: Validate request
    API->>DB: Check username unique
    DB-->>API: OK
    API->>DB: INSERT principal (status=PENDING)
    DB-->>API: principal_id
    API->>DB: INSERT principal_profile
    API->>DB: INSERT principal_tenant_membership
    API->>KC: POST /admin/realms/{realm}/users
    KC-->>API: keycloak_user_id
    API->>DB: UPDATE principal SET keycloak_user_id
    API->>MQ: Publish principal.created event
    API-->>EXT: Response: {principal_id, ...}
    
    Note over EXT: Stores principal_id in business entity
    EXT->>EXT: UPDATE employees<br/>SET principal_id = 'prin-789'
```

#### Principal Deactivation Flow
...
sequenceDiagram
    ...
    Note over HR: Employee terminated in HR system
    HR->>API: POST /principals/{id}/deactivate
    API->>DB: UPDATE principal SET status=INACTIVE
    API->>KC: PUT /users/{id} (enabled=false)
    KC->>KC: Revoke all sessions
    KC-->>API: OK
    API->>MQ: Publish principal.deactivated event
    API-->>HR: Deactivation confirmed
    
    MQ->>AUTHZ: principal.deactivated event
    AUTHZ->>AUTHZ: Revoke all role assignments
    AUTHZ->>AUTHZ: Clear permission cache
    AUTHZ->>MQ: Publish roles.revoked event
```
...
|----|------|-----------|-------------|
| BR-PRIN-001 | Username must be globally unique | Prevents ambiguity | CREATE: Database unique constraint |
| BR-PRIN-002 | Email must be globally unique (if provided) | Prevents duplicate accounts | CREATE: Database unique constraint |
| BR-PRIN-003 | principal_type is immutable | Type defines structure | UPDATE: Application validation |
| BR-PRIN-004 | username is immutable | External references | UPDATE: Application validation |
| BR-PRIN-005 | HUMAN principals must have email | Email verification required | CREATE: Application validation |
| BR-PRIN-006 | SERVICE/SYSTEM may omit email | Email not applicable | CREATE: Application validation |
| BR-PRIN-007 | keycloak_user_id is immutable after sync | Keycloak identity binding | UPDATE: Application validation |
| BR-PRIN-008 | context_tags max size 10KB | Performance | CREATE/UPDATE: Application validation |
| BR-PRIN-009 | Primary tenant membership must exist and be ACTIVE | Authorization requirement | Always: Application + DB check |
| BR-PRIN-010 | Only ACTIVE principals can authenticate | Security | Runtime: Keycloak + application |

### 4.2 Profile Rules

| ID | Rule | Rationale | Enforcement |
|----|------|-----------|-------------|
| BR-PROF-001 | Only for principal_type=HUMAN | Services don't have profiles | CREATE: Application validation |
| BR-PROF-002 | display_name is required | UI display | CREATE/UPDATE: Application validation |
| BR-PROF-003 | language must be supported | Localization | CREATE/UPDATE: Reference data check |
| BR-PROF-004 | timezone must be valid IANA | Time calculations | CREATE/UPDATE: Application validation |
| BR-PROF-005 | avatar_url must reference DMS | GDPR compliance | CREATE/UPDATE: DMS validation |

### 4.3 Service Principal Rules

| ID | Rule | Rationale | Enforcement |
|----|------|-----------|-------------|
| BR-SVC-001 | Only for principal_type=SERVICE | Type-specific entity | CREATE: Application validation |
| BR-SVC-002 | service_name must be unique | Service identification | CREATE: Database unique constraint |
| BR-SVC-003 | API keys must be hashed (SHA-256) | Security | CREATE/ROTATE: Application |
| BR-SVC-004 | Credentials rotate every 90 days | Security best practice | Background job |
| BR-SVC-005 | Plain-text API key returned only once | Security | CREATE: Application logic |
| BR-SVC-006 | allowed_scopes must be valid | Authorization | CREATE/UPDATE: Reference check |

### 4.4 System Principal Rules

| ID | Rule | Rationale | Enforcement |
|----|------|-----------|-------------|
| BR-SYS-001 | Only for principal_type=SYSTEM | Type-specific entity | CREATE: Application validation |
| BR-SYS-002 | system_identifier must be unique | System identification | CREATE: Database unique constraint |
| BR-SYS-003 | Must have certificate OR API key | Authentication required | CREATE: Application validation |
| BR-SYS-004 | certificate_thumbprint immutable | Trust establishment | UPDATE: Application validation |
| BR-SYS-005 | allowed_operations acts as whitelist | Security | Runtime: Authorization check |

### 4.5 Membership Rules

| ID | Rule | Rationale | Enforcement |
|----|------|-----------|-------------|
| BR-MEM-001 | Primary tenant membership must be ACTIVE | Authorization foundation | Always: Application validation |
| BR-MEM-002 | One ACTIVE membership per principal per tenant | Data integrity | CREATE: Database unique constraint |
| BR-MEM-003 | Membership effective between valid_from and valid_to | Temporal access control | Runtime: Application logic |
| BR-MEM-004 | If principal SUSPENDED, all memberships SUSPENDED | Cascading security | UPDATE: Application logic |
| BR-MEM-005 | valid_from <= valid_to | Logical validity | CREATE/UPDATE: Application validation |
