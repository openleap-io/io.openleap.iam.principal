...
-- Sequences for ID generation (required by PersistenceEntity)
CREATE SEQUENCE IF NOT EXISTS human_principals_seq START WITH 1 INCREMENT BY 50;
CREATE SEQUENCE IF NOT EXISTS service_principals_seq START WITH 1 INCREMENT BY 50;
CREATE SEQUENCE IF NOT EXISTS system_principals_seq START WITH 1 INCREMENT BY 50;
CREATE SEQUENCE IF NOT EXISTS device_principals_seq START WITH 1 INCREMENT BY 50;
...
(
    ...
    device_type            VARCHAR(50)         NOT NULL CHECK (device_type IN
                                                               ('IOT_SENSOR', 'EDGE_DEVICE', 'KIOSK', 'TERMINAL',
                                                                'GATEWAY', 'OTHER')),
...
CREATE
OR REPLACE VIEW iam_principal.all_principals_usernames AS
SELECT business_id, username, 'HUMAN' as principal_type
FROM iam_principal.human_principals
UNION ALL
SELECT business_id, username, 'SERVICE' as principal_type
FROM iam_principal.service_principals
UNION ALL
SELECT business_id, username, 'SYSTEM' as principal_type
FROM iam_principal.system_principals
UNION ALL
SELECT business_id, username, 'DEVICE' as principal_type
FROM iam_principal.device_principals;


CREATE
OR REPLACE FUNCTION iam_principal.update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at
= NOW();
